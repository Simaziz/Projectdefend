        ...
        <div className="flex gap-2">
          {order.status === "pending" && (
            <>
              <button onClick={() => setStatus("preparing")} disabled={isPending}
                ...
                {isPending ? <RefreshCw size={12} className="animate-spin" /> : <Flame size={12} />} Prepare
              </button>
          ...
          {order.status === "preparing" && (
            <button onClick={() => setStatus("completed")} disabled={isPending}
              ...
              {isPending ? <RefreshCw size={12} className="animate-spin" /> : <CheckCircle size={12} />} Mark Complete
            </button>
          )}
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-stone-100">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-stone-500 text-sm font-bold">Loading dashboard…</p>
        </div>
      </div>
    );
  }
              ...
              <div className="bg-white rounded-2xl border border-stone-100 shadow-sm overflow-hidden">
                ...
                <div className="px-5 py-5 space-y-4">
                  <div>
                    ...
                    <input type="text" value={settings.shopName} onChange={(e) => updateSetting("shopName", e.target.value)} placeholder="e.g. Cozy Cup Coffee"
                      className="w-full border border-stone-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" />
                  </div>
                  <div>
                    <label className="block text-xs font-black text-stone-500 uppercase tracking-wider mb-1.5">Address</label>
                    <input type="text" value={settings.shopAddress} onChange={(e) => updateSetting("shopAddress", e.target.value)} placeholder="e.g. 123 Brew Street"
                      className="w-full border border-stone-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" />
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-xs font-black text-stone-500 uppercase tracking-wider mb-1.5">Phone</label>
                      <input type="tel" value={settings.shopPhone} onChange={(e) => updateSetting("shopPhone", e.target.value)} placeholder="e.g. 081-234-5678"
                        className="w-full border border-stone-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" />
                    </div>
                    <div>
                      <label className="block text-xs font-black text-stone-500 uppercase tracking-wider mb-1.5">Email</label>
                      <input type="email" value={settings.shopEmail} onChange={(e) => updateSetting("shopEmail", e.target.value)} placeholder="e.g. hello@cozycup.com"
                        className="w-full border border-stone-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" />
                    </div>
                  </div>
                </div>
                ...
                <div className="px-5 py-4 divide-y divide-stone-50">
                  ...
                  <div className="py-3">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-lg font-black text-orange-600 bg-orange-50 px-3 py-1 rounded-xl">{settings.lowStockAlert}</span>
                    </div>
                    <input type="range" min="1" max="50" step="1" value={settings.lowStockAlert}
